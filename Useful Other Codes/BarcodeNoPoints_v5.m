function [ foundbarcodes, points, copynumfinal, rawfound, copynumfinalsum, totdropped ] = BarcodeNoPoints_v5( channels, hyb, barcodekey, PathName, posnum, segmentation, regvec,radius,conthresh,alloweddiff,scaledonoff, debug )
load([PathName '\pos' num2str(posnum) '\pos' num2str(posnum) 'PointsBarcodeAllCodes.mat']) 
if isempty(regvec)
    regvec = [0 0];
end
% if isempty(mask)
...
% 
% % for i = 1:hyb
% %     [points(i).m,points(i).dots] = findDotsBarcode(hybnum(i).color, multiplier(i,:), HCRorFISH,160*numcells,270*numcells,debug); %250-350
% % end
% 
    ...
    for k = 1:hyb
        if k ~= i
            for j = 1:channum
                ...
                calledidx = [];
                for l = 1:channum
                    [~, ~, called1, ~, pair ] = colocalizeBarcodeV2( 1,1,points, i,k, [j l],radius,debug );
