...
folders = folders(keep);
 ...
 %field = {'cpne5'; 'nes'; 'acta2';'gja1';'omg'; 'nov'; 'col5a1'; 'dcx';'itpr2';'rhob';'sox2';'cldn5'; 'mrc1'; 'tbr1';'pax6';...
    ...
    load([PathName '\' folders(1).name '\' folders(1).name 'BarcodesAllCodesRad6.mat'])
...
for i = 2:length(folders)
    load([PathName '\' folders(i).name '\' folders(i).name 'BarcodesAllCodesRad6.mat'])
    ...
    for j = 1:length(vertex)
        BW = poly2mask(vertex(j).x, vertex(j).y, 1024, 1024);
        n = regionprops(BW,'Area','Centroid');
        area(j) = sum([n.Area]);
